...
open class CloseSystemDialog : DefaultTask() {

    private val adbShell = AdbShell(project)

    @TaskAction
    fun close() {
        adbShell.forEachConnectedDeviceParallelRepeated {
    ...
    private fun dismissSystemDialog(device: IDevice) {
        ...
        adbShell.execAdbShell(device, AdbShell.StdOutLogger, "input keyevent 20") // down (get focus or select bottom button)
        adbShell.execAdbShell(device, AdbShell.StdOutLogger, "input keyevent 20") // down (select bottom button)
        adbShell.execAdbShell(device, AdbShell.StdOutLogger, "input keyevent 66") // enter (click wait/close/ok button)
    }
