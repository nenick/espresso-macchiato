...
class EspView(
    ...
    constructor(@IdRes viewId: Int = 0, interactions: EspView.() -> Unit = {})
            : this(viewId, null, interactions)

    /** Short way to locate this view by his id within different root (e.g. dialogs). */
    constructor(@IdRes viewId: Int = 0, rootMatchers: Matcher<Root>? = null, interactions: EspView.() -> Unit = {})
    ...
    private fun ensureCurrentRootIsActivity() {
        Espresso.onView(isRoot()).inRoot(not(isDialog())).noActivity()
    }
}
